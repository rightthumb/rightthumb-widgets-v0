...
up_date() {
	...
	# run with sudo if available and not already root
	maybe_sudo() {
	...
	if command -v apt-get >/dev/null 2>&1; then
		...
		echo "📦 DNF (Fedora/Alma/RHEL)"
		maybe_sudo dnf upgrade --refresh -y
		...
		maybe_sudo zypper update -y

	elif command -v apk >/dev/null 2>&1; then
		echo "📦 apk (Alpine)"
		maybe_sudo apk update
		maybe_sudo apk upgrade

	elif command -v xbps-install >/dev/null 2>&1; then
		echo "📦 xbps (Void)"
		maybe_sudo xbps-install -Syu

	elif command -v emerge >/dev/null 2>&1; then
		echo "📦 Portage (Gentoo)"
		maybe_sudo emerge --sync
		maybe_sudo emerge -avuDN @world
		...
		echo "📦 pkg (FreeBSD)"
		maybe_sudo pkg update -f
		maybe_sudo pkg upgrade -y
	...
	# Optional: update app stores if present
	if command -v snap >/dev/null 2>&1; then
		echo "📦 snap"
		maybe_sudo snap refresh
	fi

	if command -v flatpak >/dev/null 2>&1; then
		echo "📦 flatpak"
		maybe_sudo flatpak update -y
	fi
...
vscodeDocker() {
	# Usage: vscodeDocker /path/to/folder
	# Launch OpenVSCode in Docker, prompting for password

	if [ -z "$1" ]; then
		echo "❌ Error: You must provide a folder path."
		echo "Usage: vscodeDocker /path/to/folder"
		return 1
	fi
	...
	read -sp "Enter password for OpenVSCode Server: " PASSWORD
	echo

	docker run -d --name openvscode \
		-p 127.0.0.1:3000:3000 \
		-e PASSWORD="$PASSWORD" \
		-v "$FOLDER:/home/workspace" \
		linuxserver/openvscode-server
}


me() {
	# Change to the user who owns the current /home/{user}/... path
	# Usage: just run `me` while inside /home/{user}/...

	local path="$PWD"

	# Extract the username after /home/
	local user
	user=$(echo "$path" | awk -F/ '{print $3}')

	if [ -z "$user" ]; then
		echo "❌ Not inside /home/{user}/..."
		return 1
	fi
...
# Run a one-off command that won't be written to history.
# Usage:
#   shh            # prompt (echoed)
#   shh -s         # prompt (silent typing)
#   shhx ...args   # run given command/args directly with no prompt
shh() (
	# subshell: anything we change here won't leak to your main shell
	set +o history                   # disable history for this subshell
	HISTFILE=                        # no history file
	HISTCONTROL=ignorespace:erasedups
	HISTIGNORE='*'
	PROMPT_COMMAND=                  # nothing auto-appendy

	# prompt for a command
	local cmd
	if [[ $1 == "-s" ]]; then
		read -r -s -p "secret> " cmd; printf "\n"
	else
		read -r -p "hidden> " cmd
	fi
...
# Direct variant: executes arguments without ever touching history.
# Example: shhx ls -la /etc
shhx() (
	set +o history
	HISTFILE=
	HISTCONTROL=ignorespace:erasedups
	HISTIGNORE='*'
	PROMPT_COMMAND=
	# run the command exactly as passed (no eval)
	"$@"
)


alias distro.="$widgets/widgets/bash/disto_info.sh"

alias 2mp3='$widgets/widgets/bash/video2mp3.sh'


mkdir -p  $HOME/.rt/profile/FigTree/

# Create or activate a Python virtual environment
Env() {
	# Default directory for venv
	local VENV_DIR="${1:-$HOME/.venv}"

	# Expand tilde manually if passed
	...
	if [[ -f "$VENV_DIR/bin/activate" ]]; then
		...
		echo "Using Python venv at: $VENV_DIR"
	else
...
alias es="$widgets/widgets/bash/es.sh"


alias ssh.r="ssh-keygen -R "
...
sqlite_dump() {
	...
	if [ -z "$db" ]; then
		echo "Usage: sqlite_dump /path/to/database.db"
		return 1
	fi
...
sqc() {
	local db="$1"
	[ -f "$db" ] || { echo "DB not found: $db" >&2; return 1; }
	...
	while IFS= read -r tbl; do
		cnt=$(sqlite3 "$db" "SELECT COUNT(*) FROM \"$tbl\";" 2>/dev/null || echo 0)
		total=$((total + cnt))
		printf "%-30s %15s\n" "$tbl" "$(printf "%'d" "$cnt")"
	done < <(
...
alias zipFos="for d in */; do zip -r "${d%/}.zip" "$d"; done"



alias clip.="echo '' > /tmp/clip; nano /tmp/clip; ttcat /tmp/clip"
alias pclip="echo '' > /tmp/clip; nano /tmp/clip; ttcat /tmp/clip"
alias pfo=clip. | p sfolder + [pipe] -or"
alias pfi=clip. | p sfile + [pipe] -or"
alias cfo=clip. | p sfolder + [pipe] -or"
alias cfi=clip. | p sfile + [pipe] -or"

alias us="cut -d: -f1 /etc/passwd | p sfolder + [pipe] -or"
alias users.="cut -d: -f1 /etc/passwd | p sfolder + [pipe] -or"
alias users.p="cut -d: -f1 /etc/passwd | p sfolder + [pipe] -or -p"
alias users.path="cut -d: -f1 /etc/passwd | p sfolder + [pipe] -or -p"

alias ctrl+shift+b="echo '' > ~/.bashrc; python3 /opt/rightthumb-widgets-v0/install/installer.py -rc.d h ; m loading-bookmarks"
alias csb="echo '' > ~/.bashrc; python3 /opt/rightthumb-widgets-v0/install/installer.py -rc.d h ; m loading-bookmarks"
alias bashrc="echo '' > ~/.bashrc; python3 /opt/rightthumb-widgets-v0/install/installer.py -rc.d h ; m loading-bookmarks"

alias bm.xfer="$p hasAlias -xfer"
alias bmx="$p hasAlias -xfer"
...
alias addBlock="/opt/rightthumb-widgets-v0/widgets/bash/addBlock.sh"
