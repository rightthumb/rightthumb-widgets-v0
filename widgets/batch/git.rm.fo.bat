...
if not [%1] == [] (
	echo No, you can not specify a folder.
	echo   You have to be in that folder and it deletes THE FOLDER YOU ARE IN.
	goto :eof
)

@REM if not "%1" == "run" echo must be in folder you want to remove. run: git.rm.fo run
@REM if not "%1" == "run" goto :eof

@REM :: Define the temporary storage path if not already defined
@REM if not defined stmp set "stmp=%TEMP%"
...
@REM echo git filter-branch --force --index-filter "git rm -r -f --cached --ignore-unmatch \"%full_path%\"" --prune-empty --tag-name-filter cat -- --all
echo git rm -r -f "%full_path%"
set "shouldRun="
