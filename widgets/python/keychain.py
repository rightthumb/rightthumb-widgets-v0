...
def appSwitches():
	...
	_.switches.register( 'SSH', '-ssh' )
	_.switches.register( 'NoWSL', '-n,-nowsl' )
	_.switches.register( 'Silent', '--c,-silent' )
	_.switches.register( 'DoNotOverwriteOnAdd', '--n,-on,--only-new' )
...
def add():
	...
	if _.switches.isActive('Alias'):
		for alias in _.switches.values('Alias'):
			aliases[alias] = label
		_.saveTable( aliases, 'keychain-aliases.hash' )
		return None
	...
	try:
		if _.switches.isActive('DoNotOverwriteOnAdd') and label in table:
			_.pr('skipping:',label,c='yellow')
		else:
			_.pr('adding:',label,c='green')
			table[label] = _vault.imp.s.en( password )
	except: _.e( 'Login error' )
	# if 'eof' in table: del table['eof']; table['eof']='';
	_.saveTable( table, 'keychain.hash' )
...
def action():
	if _.switches.isActive('SSH'):
		cmd = ''
		if _.isWin and not _.switches.isActive('NoWSL'):
			cmd = 'wsl '
		label = _.switches.values('Label')[0]
		pw = get( label, cp=False )
		cmd +'sshpass -p '+pw+' '+'ssh '+label
		os.system( cmd )
		return None 
...
def load():
	...
	if table is None:
		table = _.getTable( 'keychain.hash' )
		aliases = _.getTable( 'keychain-aliases.hash' )
