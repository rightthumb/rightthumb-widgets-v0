...
def action():
	...
	for file in os.listdir( folder ):
		if os.path.isfile(file):
			files.append({  'name': file, 'path': folder+_v.slash+file, 'in': False })
	...
	for i,row in enumerate(_.isData(r=1)):
		if _.showLine( row ):
			...
			if _.switches.isActive('Split'):
				if not _.switches.values('Split')[0] in row:
					subject = []
				elif _.switches.values('Split')[0] in row:
					if len(_.switches.values('Split')) > 1:
						subject = [ row.split( _.switches.values('Split')[0] )[ int( _.switches.values('Split')[1] ) ] ]
					else:
						subject = row.split( _.switches.values('Split')[0] )

			found = False
			for sub in subject:
				for i,file in enumerate(files):
					if sub.lower() in file['name'].lower():
						found=True
						files[i]['in'] = True
			if found:
				subjects.append({ 'row': row, 'subjects': subject, 'in': True })
			else:
				subjects.append({ 'row': row, 'subjects': subject, 'in': False })
