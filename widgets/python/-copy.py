...
def subject():
		...
		subject = {}
		subject['py'] = {}
		subject['py']['args'] = ''' args, unknown = ap.parse_known_args(); leftover = [u for u in unknown if u.startswith("-")] # ap.add_argument '''.strip()
...
# _.pr( str( '\\n'.join( dir(_v) )  ) )
		...
		# print(ssh)
		subject['ssh'] = {}
		subject['ssh']['open'] = 'p file-open -a ssh -f /etc/ssh/sshd_config'
		if 'IPs' in _ssh: subject['ssh']['ips'] = _ssh['IPs']
		subject['ssh']['keepalive'] = '''
# Server
## Don’t ever drop the client due to missed keepalives
ClientAliveInterval 60
ClientAliveCountMax 0

# Client
## Don’t ever drop the server due to missed keepalives
ServerAliveInterval 60
ServerAliveCountMax 0

		'''.strip()
		subject['ssh']['persist'] = subject['ssh']['keepalive']
