...
_.appInfo[focus()] = {
	'file': 'extractFromExcel.py',
	'description': 'Extract data from csv by year',
	'categories': [
						'project',
						'csv',
				],
...
_.appInfo[focus()]['relatedapps'].append('p crossRefCSV -i one.csv two.csv')
_.appInfo[focus()]['examples'].append('p extractFromExcel -file file.csv')
...
def action():
	if _.switches.isActive('Input'):
		csvFile = _.getText( _.switches.value('Input') )
		result = {}
		keys = []
		year = '2008'
		keys.append( year )
		...
		for i,y in enumerate(keys):
			try:
				_.saveText(result[y], name[i] + '.csv' )
