...
def appSwitches():
	_.switches.register( 'Dice', '-dice' )
	_.switches.register( 'Count', '-count' )
	_.switches.register( 'Advantage', '-adv' )
	_.switches.register( 'Disadvantage', '-dis' )
	_.switches.register( 'Threshold', '-t' )
	pass
	### EXAMPLE: START
...
_.appInfo[focus()] = {
	...
	'examples': [
						_.hp(''),
						_.hp('p dice -count 1024 -adv -t 13'),
						_.hp(''),
						_.hp('p dice -count 20 -dice 100'),
						_.hp(''),
						'',
	],
	'columns': [
...
def action():
	...
	if _.switches.isActive('Threshold'):
		threshold=int(_.switches.value('Threshold'))
	rolls = 1
	if _.switches.isActive('Dice'):
		rolls = int(_.switches.value('Dice'))
	if _.switches.isActive('Count'):
		rolls = int(_.switches.value('Count'))

	adv=_.switches.isActive('Advantage')
	dis=_.switches.isActive('Disadvantage')
	thresh=_.switches.isActive('Threshold')
