...
_.appInfo[focus()] = {
	...
	'description': 'Multi-lines to single line',
	'categories': [
						'tool',
						'build',
						'make',
	...
	'examples': [
						"type %tmpf6% | p line -p ' 1 | p single -make '{}', ",
						"",
						"type %tmpf6% | p line -p ' 1 | p single -make '{}': row.{},",
						"",
						'',
	],
	'columns': [
...
def process( row ):
	...
	if _.switches.isActive('Make'):
		result = ' '.join( _.switches.values('Make') ).replace( '{}', row )
	return result
