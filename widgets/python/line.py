...
def _pr0(*args, **kwargs):
	Strip = False
	if _.switches.isActive('Make') or _.switches.isActive('Parse') or _.switches.isActive('ParseSearch') or _.switches.isActive('MultiParseSearch') or _.switches.isActive('ParsePrint') or _.switches.isActive('ParseClean') or _.switches.isActive('Parse2') or _.switches.isActive('ParseSearch') or _.switches.isActive('ParsePrint'):
		Strip = True
		y = []
		for x in args: y.append(str(x).replace(__.ParseFix,'').strip())
		args = y
	args = tuple(args)
	kwargs['Strip'] = Strip
	return _.pr(*args, **kwargs)
...
def _pr(*args, **kwargs):
	...
	if _.switches.isActive('Make') or _.switches.isActive('Parse') or _.switches.isActive('ParseSearch') or _.switches.isActive('MultiParseSearch') or _.switches.isActive('ParsePrint') or _.switches.isActive('ParseClean') or _.switches.isActive('Parse2') or _.switches.isActive('ParseSearch') or _.switches.isActive('ParsePrint'):
		Strip = True
		def Trigger(string):
			return string.replace(__.ParseFix,'').strip()
		trigger = Trigger
	args = tuple(args)
	if not type(kwargs) == dict: kwargs = {}
	# kwargs['Strip'] = Strip
...
def appSwitches():
	...
	_.switches.register('Parse', '--p,-p,-parse,-split',';; 0;2 or ;sp 2 ')
...
if _.switches.isActive('Make'):
	...
	if '{}' in _isData or '{a}' in _isData or '{0}' in _isData or '{1}' in _isData:
		...
		if '{}' in _isData:
			for line in isData: _pr(line.replace(__.ParseFix,'').replace('{}',make[0]))
		else:
...
if _.switches.isActive('PrintDelimLines'):
	_pr( '__________________________________________________________________________________' )
	_pr( '____________________ ____________________ ____________________ ____________________' )
	sys.exit()
...
def displayLine(line):
	...
	if _.switches.isActive('ParseSearch'):
		...
		line = line.replace(p[0],__.ParseFix)
		p[0] = __.ParseFix
...
def crossReference(inLine,file,xRefRev):
	...
	if match == True and matchType == 'complex':
		if displayThis == 3:
			_pr(result['files'][0]['path'],result['files'][1]['path'])
		elif displayThis == 1:
			_pr(result['files'][0]['path'])
		elif displayThis == 2:
			_pr(result['files'][1]['path'])
		elif displayThis == 0:
			_pr(result['negotiated'])
		elif displayThis == 4:
...
if _.switches.isActive('CrossReferenceReverse') == False:
	...
	for theLineNumber,line in enumerate( _.pp() ):
		...
		if _.switches.isActive('Plus') and len(_.switches.records('relevant'))==1:
			if _.showLine(line): _pr(line.replace(__.ParseFix,'')); cnt+=1; continue;
					...
					if _.switches.isActive('Original'):
						if _.switches.isActive('Plus'):
							...
							for plusSearchX in _.switches.values('Plus'):
								...
								#         _pr( len(plusSearchX)-1, len(thePrintLine)-1, scanning )
						...
						# _pr(_.inlineBold(theLineNumber+1),thePrintLine)
						_pr(_.inlineBold(theLineNumber),thePrintLine)
					else:
						if _.switches.isActive('Plus'):
							thePrintLine = line
							for plusSearchX in _.switches.values('Plus'):
...
if _.switches.isActive('CrossReference') == True and _.switches.isActive('NoCount') == False  and _.switches.isActive('CrossReferenceUltimate') == False:
	_pr('\tGood',cntCrossRefG1,'\tBad',cntCrossRefB1)
	_pr('Total',cntCrossRefB1 + cntCrossRefG1)
...
if _.switches.isActive('CrossReferenceReverse') == True:
	...
	if _.switches.isActive('NoCount') == False:
		...
		if _.switches.isActive('CrossReferenceUltimate') == False:
			_pr('\tGood',cnt2,'\tBad',cntCrossRefB2)
			_pr('Total',cntCrossRefB2 + cnt2)
		else:
			...
			_pr('\tGood',cntGood,'\tBad',linesPipe - cntGood)
			_pr('Pipe',linesPipe,'Lines\n')
			_pr('\tGood',cntGood,'\tBad',linesFile - cntGood)
			_pr('File',linesFile,'Lines\n')
