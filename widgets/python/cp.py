...
_.switches.register('One', '-1,-two','file.txt')
_.switches.register('Two', '-2,-two','file.txt')
...
_.appInfo[__name__]['examples'].append('p cp -i file1.txt file2.txt')
...
# _.showLine(item)
########################################################################################
import txtBackup2 as fileBackup
...
def action():
	...
	if _.switches.isActive('Input'):
		...
		f = files.split(',')
		_.switches.fieldSet('One','active',True)
		_.switches.fieldSet('One','value',f[0])
		_.switches.fieldSet('Two','active',True)
		_.switches.fieldSet('Two','value',f[1])



	if _.switches.isActive('One') and _.switches.isActive('Two'):
		files = _.switches.value('One') + ',' + _.switches.value('Two')
		print(files)

		f = [_.switches.value('One'),_.switches.value('Two')]

		fileBackup.focus()
		_.switches.fieldSet('Input','active',True)
		_.switches.fieldSet('Input','value', f[0])
		fileBackup.action()
		_.switches.fieldSet('Input','value', f[1])
		fileBackup.action()
		...
		do = 'type "' + f[0] + '" > "' + f[1] + '"'
		# mime0 = mimetypes.guess_type(f[0])
		# mime1 = mimetypes.guess_type(f[1])
		# print(mime0)
		# print(mime1)
		# if str(mime0) == "('text/plain', None)" and str(mime1) == "('text/plain', None)":
		if isText(f[0]) and isText(f[1]):
			try:
				# result = copyfile(f[0], f[1])
				result = os.system('"' + do + '"')
				print('type')
			except Exception as e:
				result = 'Error'
