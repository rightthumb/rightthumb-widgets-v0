...
def touchpad_toggle():
	...
	INSTANCE_ID = r'HID\MSNB0001&COL02\5&581BF97&0&0001'
	STATE_FILE = Path.home() / ".msi_touchpad_state"

	# Determine next state
	if STATE_FILE.exists():
		state = STATE_FILE.read_text(encoding="utf-8").strip().lower()
		next_enabled = state != "enabled"   # if enabled → disable, else enable
	else:
		next_enabled = False  # default: assume touchpad is ON → disable first

	# PowerShell command
	action = "Enable-PnpDevice" if next_enabled else "Disable-PnpDevice"
	ps_cmd = f'{action} -InstanceId "{INSTANCE_ID}" -Confirm:$false'
	...
	# Save new state
	STATE_FILE.write_text("enabled" if next_enabled else "disabled", encoding="utf-8")

	return next_enabled  # True = enabled, False = disabled
...
class HOTKEYS:
	...
	def release_key( self, key ):
		...
		global post_do
		if not type(post_do) is dict: post_do = {}
		global key_set
		global force_clean
		global no_escape
		if post_do and type(post_do) is dict and 'PreHook' in post_do:
		...
		if post_do and not key_set and post_do['status']:
			post_do['status'] = 0
			count = post_do['count']
			do = post_do['do']
			k = post_do['label']
...
class CLIP:
	...
	def _save_clip_(slot='0'):
		_.pr('_save_clip_',slot)

		slot=str(slot)
		fi = _v.stmp+os.sep+'hotkeys-save_clip_'+slot
		print(fi)

		# path   = _v.stmp+os.sep+'hotkeys-save_clip_'
		# pathBK = path
		# path0 = path + '0'
		# if n == 0 or n == '0' or str(n).strip() == '':
		# 	fi = path0
		# else:
		# 	fi = path + str(n)
		# if path == pathBK:
		# 	path = path0
		_paste = _.regImp( __.appReg, '-paste' ); _paste.imp.dirty=True; 

		_.pr('To toggle paste: clip-auto', slot, fi, c='yellow')
		clip = _paste.imp.paste()
		_.saveText(clip,fi)
		_.pr('Saved to:',fi,c='green')
		
		_.pr(line=1,c='green')
		# _.pr(clip[:1000],c='cyan')
		_.pr(clip,c='cyan')
		_.pr(line=1,c='green')
	
	# load_clip_3
	def save_clip_0__OLD(self):
		...
		secure = ''
		for n in [1, 2, 3, 4, 5, 6, 7, 8, 9]: n = str(n); _.saveText(secure,_v.stmp+os.sep+'hotkeys-save_clip_'+n);
	def save_clip_0(self): CLIP._save_clip_('0')
	def save_clip_1(self):  CLIP._save_clip_('1')
	def save_clip_2(self):  CLIP._save_clip_('2')
	def save_clip_3(self): CLIP._save_clip_('3')
	def save_clip_4(self): CLIP._save_clip_('4')
	def save_clip_5(self): CLIP._save_clip_('5')
	def save_clip_6(self): CLIP._save_clip_('6')
	def save_clip_7(self): CLIP._save_clip_('7')
	def save_clip_8(self): CLIP._save_clip_('8')
	def save_clip_9(self): CLIP._save_clip_('9')
	...
	def _load_clip_(self,slot='0'):
		slot=str(slot)
		auto = _v.stmp+os.sep+'hotkeys-save_clip-auto-paste';
		...
		if os.path.isfile(fi):
			...
			data = ''
		if not data:
			_.pr('No data saved in slot',slot,c='red')
			return None
	...
	def load_clip_0(self): self._load_clip_('0')

	def load_clip_1(self): self._load_clip_('1')
	def load_clip_2(self): self._load_clip_('2')
	def load_clip_3(self): self._load_clip_('3')
	def load_clip_4(self): self._load_clip_('4')
	def load_clip_5(self): self._load_clip_('5')
	def load_clip_6(self): self._load_clip_('6')
	def load_clip_7(self): self._load_clip_('7')
	def load_clip_8(self): self._load_clip_('8')
	def load_clip_9(self): self._load_clip_('9')
	...
	def keychain(self):
		...
		_keychain.switch('Silent')
		__.keychain_copy=True
		_paste = _.regImp( __.appReg, '-paste' )
		key = _paste.imp.paste()
		key = key.replace(' ','')
		pw = _keychain.imp.key(key)
		_copy = _.regImp( __.appReg, '-copy' )
		_copy.imp.copy( str(pw).strip(), p=0 )
		time.sleep(5)
		beepy.simple_beep2()
		_copy.imp.copy( key, p=0 )
...
def load():
	...
	table = {
				...
				'load_clip_4': { 'raw': [ 'esc.,2', '4' ], 'do': 'Clip.load_clip_4()' },
				'load_clip_5': { 'raw': [ 'esc.,2', '5' ], 'do': 'Clip.load_clip_5()' },
				'load_clip_6': { 'raw': [ 'esc.,2', '6' ], 'do': 'Clip.load_clip_6()' },
				...
				'keychain': { 'raw': [   'alt.,2', 'x'   ], 'do': 'Clip.keychain()', 'no-esc': True, 'esc': False,  },
				...
				# 'FlagFileFolder': { 'raw': [   'ctrl.',  'space.'    ], 'do': 'FlagFileFolder()' },
				...
				'touchpad_toggle': { 'raw': [   'alt.',     'up.'   ], 'do': 'touchpad_toggle()'  },
	...
	for k in table:
		...
		for key in  table[k]['raw']:
			...
			t = t.replace('f1.',  'Key.f1')
			t = t.replace('f2.',  'Key.f2')
			t = t.replace('f3.',  'Key.f3')
			t = t.replace('f4.',  'Key.f4')
			t = t.replace('f5.',  'Key.f5')
			t = t.replace('f6.',  'Key.f6')
			t = t.replace('f7.',  'Key.f7')
			t = t.replace('f8.',  'Key.f8')
			t = t.replace('f9.',  'Key.f9')
			t = t.replace('f10.', 'Key.f10')
			t = t.replace('f11.', 'Key.f11')
			t = t.replace('f12.', 'Key.f12')

			t = t.replace('up.',    'Key.up')
			t = t.replace('down.',  'Key.down')
			t = t.replace('left.',  'Key.left')
			t = t.replace('right.', 'Key.right')
...
def do(args,kwargs):
	pass
	_.pr(line=1)
	_.pr('listen:',args,kwargs,c='yellow')
	_.pr(line=1)
	# eval( args )
