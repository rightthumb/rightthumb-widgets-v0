...
def action():
	...
	if not type( _.appData[__.appReg]['pipe'] ) == bool:
		...
		for row in _.appData[__.appReg]['pipe']:
			try:
				info = _inDic.imp.testAll( row )
				record = { 'word': row, 'profile': info }
				data.append( record )
				if not info in combos:
					combos.append( info )
		...
		_.saveTable( data, 'dictionary_words_profile.json' )
		_.saveTable( combos, 'dictionary_profile_combinations.json' )
