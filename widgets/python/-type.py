...
def sw():
    ...
    _.switches.register( 'AddSpace', '-a,--s,-space' )
...
def type_text(text: str, wait_seconds: float = 0.5, cps: int = 20):
    """
    Simulate typing text into the currently active window on Windows.
    Works on VNC/RDP terminals that don't allow paste.

    Args:
        text (str): Multiline string to type (newlines will send Enter).
        wait_seconds (float): Seconds to wait before starting typing.
        cps (int): Characters per second typing speed.
    """
    if wait_seconds > 0:
...
if __name__ == "__main__":
    if _.isData(2):
        ...
        if _.switches.isActive('AddSpace'):
            demo_text = ' '+_paste.imp.paste().replace('\r','')
    ...
    if not len(ask):
        ...
        # int(ask)
        wait_seconds = 3
        print(f"Typing will start in {wait_seconds}s.")
        time.sleep(3)
        type_text(demo_text, wait_seconds=1, cps=250)
    else:
        print('skipped')
