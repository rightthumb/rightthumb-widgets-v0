...
# _ent = _.regImp( __.appReg, 'movieTitle' )
import movieTitle
...
def action():
	...
	for i,record in enumerate(data):
		if not len(record['label']):
			# file = record['path'].split('\\').pop()
			file = record['data']['name']
			label = movieTitle.clean( file )
			data[i]['label'] = label
			record['label'] = label
	
	_.saveTableDB( data, 'franchise_file_data.json' )
...
def load():
	global data
	data = _.getTableDB( 'franchise_file_data.json' )
