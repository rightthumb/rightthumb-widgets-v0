...
def attrib(path, a=None, s=None, h=None, r=None, i=None):
	global shouldAquireAttribute
	if not shouldAquireAttribute:
		return ''
	attrs=[]
		...
		if os.path.isdir(path):
			...
			if isSwitchActive('Recursion') and timeoutKill == False and getSubFolderData:
				...
				if modifiedRaw > 100:
					...
					woy_cd = datetime.datetime.fromtimestamp( int(createdRaw) ).isocalendar()[1]
					weekAndYear_cd = round(woy_cd * 0.01,2) + year
				...
				if isSwitchActive('Database'):
					cursor.execute("INSERT INTO files VALUES ('"+str(path2)+"', '"+str(fileNameLength(item, ''))+"', '"+str(item)+"', '"+str(folder)+"', '', '', '"+str(int(size))+"', '"+str(sizeF)+"', '"+str(int(createdRaw))+"', '"+str(int(modifiedRaw))+"', '"+str(created)+"', '"+str(modified)+"', 'Folder', '0', '', '"+woyPad(weekAndYear)+"', '"+str(woy)+"', '"+str(dow)+"', '"+str(month)+"', '"+str(friendlyWeek1)+"', '"+str(friendlyMonth1)+"')")
					conn.commit()
					if isSwitchActive('MD5'):
						...
						cursor2.execute("INSERT INTO md5 VALUES ('"+str(md5)+"', '"+str(path2)+"',  '"+str(item)+"', '"+str(folder)+"', '"+int(size)+"', '"+str(int(createdRaw))+"', '"+str(int(modifiedRaw))+"')")
						conn2.commit()
					...
					if isSwitchActive('Database'):
						...
						if not item == defaultCachFile:
							dirRows.append( {
												...
												'typesort': 1,
												'ext': ext.upper(),
												'week_of_year_cd': woyPad(weekAndYear_cd),
												'week_of_year_cd_': woy_cd,
...
def saveTable2( rows, theFile, tableTemp=False, printThis=False, indentCode=True, sort_keys=False ):
	import _rightThumb._vars as _v
	# defaults to myTables
	if indentCode:
		dataDump = json.dumps(rows, indent=4, sort_keys=sort_keys)
...
def columnNickname1(column):
	...
	try:
		...
		if column == 'week_of_year_cd_':
			column = 'date_created_raw'
		if column == 'week_of_year_cd':
			column = 'date_created_raw'
...
def columnNickname2(column):
	...
	try:
		...
		if column == 'woyc' or column == 'woycd' or column == 'wc':
			column = 'week_of_year_cd'
		if column == 's':
			column = 'size'
		if column == 't':
			column = 'type'
...
def takeAction():
	...
	if isSwitchActive('Help'):
		...
		colorizeRow('\t\t\t\t mData',2)
		colorizeRow('\t\t\t\t p dir -cache %mData% -movietitle -movies -franchise all save',2)
		colorizeRow('\t\t\t\t p franchise_label_fix',2)
		# colorizeRow('',2)
		...
		colorizeRow('\n\t',2)
		printBold('\n\tp dir -g wc -c ext wc cd n s -ago 3w cd','red')
		colorizeRow('\n\t',2)
		print('\n\t'+inlineBold('p dir -cache %mData% -movietitle -movies -franchise marvel ','red'))
		colorizeRow('\n\t p dir -cache %mData% -movietitle -movies -franchise marvel ',2)
		colorizeRow('\n\t p dir -cache %mData% -movietitle -movies -ago 2w ',2)
...
# 
					...
					if isSwitchActive('Cache'):
						if getSwitchValue('Cache') == _v.databank+_v.slash+'movie.cache':
							imdb.fieldSet( 'Location', 'active', True )
							imdb.fieldSet( 'Location', 'value', 'Movie_Drive' )
						if getSwitchValue('Cache') == _v.databank+_v.slash+'movie-cloud.cache':
							imdb.fieldSet( 'Location', 'active', True )
					...
					# if 'id' in getSwitchValue('MovieFranchise') or 'all' in getSwitchValue('MovieFranchise'):
						# shouldRun = True
						# printFranchiseTable
						franchise_file_data = getTable2( _movieFranchiseCache )
						franchiseData = []
						inList = []
						if ( 'id' in getSwitchValue('MovieFranchise') or 'all' in getSwitchValue('MovieFranchise')  ):
					...
					if shouldRun:
						...
						if 'save' in getSwitchValue('MovieFranchise'):
							saveTable2( results, _movieFranchiseCache )
				else:
...
def getTable2(theFile,printThis = False):
	if printThis:
		print('Loaded: ' + theFile)
	if os.path.isfile(theFile):
...
_movieFranchiseCache = _v.dbTables + _v.slash+'franchise_file_data.json'
...
switch.append({'name': 'Folder','switch': '-f,-folder,+or,-searchor,-so', 'pos': None, 'active': False, 'value': None, 'expected_input_example': 'E:\\tech\\programs\\python\\src\\windows \\tech Y:'})
...
switch.append({'name': 'LongFileName','switch': '-l,-ln,-long,-long', 'pos': None, 'active': False, 'value': None, 'expected_input_example': None})
switch.append({'name': 'ShortFileName','switch': '-sn,-short', 'pos': None, 'active': False, 'value': None, 'expected_input_example': None})
...
switch.append({'name': 'Pass','switch': '-pass', 'pos': None, 'active': False, 'value': None, 'expected_input_example': None})
switch.append({'name': 'GroupSegment','switch': '-segment', 'pos': None, 'active': False, 'value': None, 'expected_input_example': None})
#################################################################
#################################################################
