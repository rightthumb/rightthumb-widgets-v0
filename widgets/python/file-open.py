...
def registerSwitches( argvProcessForce=False ):
	...
	_.switches.trigger( 'Alias', _.resolveFoAliasWithFile )
...
def action(path=None):
	...
	if _.switches.isActive('Alias') and not _.switches.isActive('Files') and len(_.switches.values('Alias')) ==1:
		...
		# if '==' in _path_:
			# _path_ = _.resolveFoAliasWithFile(_path_)
			# print(_path_)
		if _path_.startswith('http:') or _path_.startswith('https:'):
			_path_=_.url2file(_path_)
			# print('here'); sys.exit();
	...
	if paths:
		...
		for path in paths:
			...
			_.append(path,_v.tt+os.sep+'file-open.last.log')
			logPathFo = _v.tt +os.sep + 'file-open' + os.sep + _.day() + os.sep 
			_v.mkdir( logPathFo )
			logPath = logPathFo + os.sep+  'today.files.log'
			_.append(path,logPath) 
...
########################################################################################
if __name__ == '__main__':
	if '--open' in sys.argv and os.path.isfile(sys.argv[sys.argv.index('--open')+1]):
		justOpen(sys.argv[sys.argv.index('--open')+1])
		sys.exit()
	else:
		action()
	__.isExit()
