...
def appSwitches():
	...
	_.switches.register( 'Files', '-f,-file,-files','file.csv', description='Files' )
	_.switches.register( 'GlobFiles', '-in,-glob','*.csv', description='Files' )
...
def action():
	...
	for file in files:
		for record in _.csv( file ):
			data['email'][record['Email']] = []

	for file in files:
		for record in _.csv( file ):
			# data['file'][file].append( record )
			record['file'] = file
			data['email'][record['Email']].append( record )

	cnt = 0
	for key in data['email'].keys():
		if len( data['email'][key] ) > 1:
			cnt += 1
...
def load():
	global files
	if _.switches.isActive('GlobFiles'):
		files = glob.glob( _.switches.values('GlobFiles')[0] )
