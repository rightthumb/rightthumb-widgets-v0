...
def action():
	...
	for x in spells_raw:
		record = {
					'name': x['spell'],
					'level': x['level'],
					'casting_time': _str.cleanBE(  x['fields'][0].split(':')[1]  , ' ' ),
					'range': _str.cleanBE(  x['fields'][1].split(':')[1]  , ' ' ),
					'components': _str.cleanBE(  x['fields'][2].split(':')[1]  , ' ' ),
					'duration': _str.cleanBE(  x['fields'][3].split(':')[1]  , ' ' ),
					'classes': _str.cleanBE(  x['fields'][4].split(':')[1]  , ' ' ),
					'description': x['fields'][5]
		}
		records.append( record )
	_.saveTableProject( 'DnD.roll20.spells', records, 'spells.json' )
def load():
	global spells_raw
	spells_raw = _.getTableProject( 'DnD.roll20.spells', 'spells_raw.json' )
