...
def appSwitches():
	...
	_.switches.register( 'Files', '-f,-file,-files','app.py', isPipe='name', description='Files' )
	### EXAMPLE: END
	...
	for i,row in enumerate(_.isData(r=1)):
		...
		for record in backupLog:
			...
			if file.lower() == record['file'].lower() or fileW.lower() == record['file'].lower():
				if record['timestamp'] > match['epoch']:
					...
					match['record'] = record
		if not found:
			backupLog = _.getTable2('D:\\tech\\hosts\\MSI\\tables\\fileBackup.json')
			for record in backupLog:
				if file.lower() == record['file'].lower() or fileW.lower() == record['file'].lower():
					if record['timestamp'] > match['epoch']:
						found = True
		...
		if found:
			...
			if _.isWin:
				...
				subprocess.call([ 'diff', file, match['record']['backup']])

		else:
			_.colorThis( 'not in backup' )
