...
def appSwitches():
	_.switches.register( 'Files', '-f,-file,-files','file.json', isPipe=True, isRequired=True, description='Files' )
...
def action():
	...
	for key in data:
		...
		if key in omit:
			label = key + '_SAFE'
			_.colorThis( [ 'Error:', key, 'in omit list' ], 'red' )
			_.colorThis( [ '\tChanged to:', label ], 'green' )

		var = 'SET ' + label + '=' + str(data[key])
		file += var + '\n'
...
def load():
	...
	omit = []
	for ot in theOmitList.split('\n'):
		if len(ot):
			omit.append( ot.lower() )
