...
def appSwitches():
	_.switches.register( 'Title', '-t,-title', '$( whoami ) $prompt_symbol bespin' )
...
def action():
	...
	if _.switches.isActive('Title') and len(_.switches.value('Title')):
		title = ' '.join(_.switches.values('Title'))
		if _.isWin:
			cmd='title '+title
		else:
