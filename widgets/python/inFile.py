...
_.appInfo[focus()] = {
	'file': 'inFile.py',
	'description': 'Find pipe data in file',
	'categories': [
						'research',
				],
	'relatedapps': [],
...
_.appInfo[focus()]['examples'].append('type %tmpf1% | p inFile -i file.txt')
...
def action():
	if _.switches.isActive('Input') and not type( _.appData[__.appReg]['pipe'] ) == bool:
		file = _.getText( _.switches.value('Input') )
		file = _.cleanList( file )
