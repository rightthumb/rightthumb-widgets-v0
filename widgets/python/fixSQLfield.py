...
_.switches.register('FieldNumber', '-fn,-fieldnumber')
...
_.appInfo[__name__] = {
	'file': 'fixSQLfield.py',
	'description': 'take sql dump and fix a field',
	'prerequisite': [],
	'examples': [],
	'columns': [],
	}

_.appInfo[__name__]['examples'].append('p fixSQLfield -fn 6 -i data.sql')
_.appInfo[__name__]['examples'].append('p fixSQLfield -fn 6 -i %tmpf3%')
...
def modColumn(row):
	...
	result = ''
	columns = row.split(',')
	fn = int(_.switches.value('FieldNumber'))
	success = False
	for i,column in enumerate(columns):
		if column.startswith(' '):
			column = _str.cleanFirst(column,' ')
			startspace = True
			result += ' '
		else:
			startspace = False
	...
	if success and _.switches.isActive('Test'):
		result = str(totalcount) + '**' + result
	return result

def action():
	global totalcount
	sql = _.getText(_.ci(_.switches.value('Input')))
	for row in sql:
		row = row.replace('\n','')
		row = modColumn(row)
