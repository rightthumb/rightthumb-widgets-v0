...
def hexColor(*args, h=None, bg=None, c=None, p=False,sp=' '):
	text = sp.join(map(str, args))
	global color_dict
	if type(c) == str:
		c = ' '.join(c.strip().replace('\t',' ').split()).replace(' ', ',')
	if type(h) == str:
		h = ' '.join(h.strip().replace('\t',' ').split()).replace(' ', ',')
	...
	if bg_color:
		...
		br, bg_g, bb = (int(bg_color[i:i+2], 16) for i in (0, 2, 4))
		colorized = f"\033[38;2;{r};{g};{b}m\033[48;2;{br};{bg_g};{bb}m{text}\033[0m"
	else:
		colorized = f"\033[38;2;{r};{g};{b}m{text}\033[0m"
