...
class MdFig:
	...
	def _normalize_markdown(self, md):
		"""
		Normalize Markdown so MdFig.parse() can consume it safely without altering language tags.

		Key points:
		- Convert any code fences that start with backticks or tildes to use '~~~'.
		- Preserve the *exact* language string (no lowercasing or renaming), but trim surrounding spaces.
		- Emit opening fences as '~~~<language>' (no space before the language) so parse() sees it via stripped_line[3:].
		- Emit closing fences as exactly '~~~'.
		- Do not modify lines inside fenced code blocks.
		- Normalize headings to ensure a space after '#'.

		Returns: list[str] suitable for self.md
		"""
		import re

		text = '\n'.join(md) if isinstance(md, list) else (md or '')
		text = text.replace('\r\n', '\n').replace('\r', '\n').lstrip('\ufeff')
		...
		# Match a fence line when stripped: ```lang / ```` / ~~~lang / ~~~~~ etc.
		# Group 1 = fence chars, Group 2 = optional language (kept verbatim, just trimmed)
		fence_re = re.compile(r'^([`~]{3,})\s*([^\s]*)\s*$')
		# Ensure headings like "###Title" become "### Title"
		heading_nospace_re = re.compile(r'^(#{1,6})(\S)')

		for line in lines:
			if in_fence:
				# Only check the stripped line to detect a closing fence; do not alter inner content.
			...
			# Handle opening/standalone fence
			m = fence_re.match(line.strip())
			if m:
				lang = (m.group(2) or '').strip()  # preserve case/content; just trim spaces
				out.append('~~~' + (lang if lang else ''))
				in_fence = True
				continue

			# Normalize headings: ensure a space after hashes
			mh = heading_nospace_re.match(line)
			if mh:
				line = mh.group(1) + ' ' + line[len(mh.group(1)):]
				out.append(line)
				continue

			out.append(line)

		# If file ended mid-fence, close it to avoid dangling state
		if in_fence:
			out.append('~~~')
