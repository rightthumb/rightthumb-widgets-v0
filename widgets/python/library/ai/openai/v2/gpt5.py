...
class GPTClient:
    ...
    # NEW: session inspection utilities (parity with PHP getSessionInfo / listSessions)
    def get_session_info(self, sid: t.Optional[str] = None) -> dict:
        """
        If sid is provided, return summary info for that session.
        If sid is None, return a mapping of sid -> info for all sessions.
        """
        if sid is not None:
            self._ensure_session(sid)
            return {
                "id": sid,
                "label": self.session_labels.get(sid),
                "parent": self.session_parents.get(sid),
                "children": list(self.session_children.get(sid, [])),
                "messages": len(self.sessions.get(sid, [])),
                "vectors": len(self.session_vecs.get(sid, [])),
                "checkpoints": len(self.session_checkpoints.get(sid, [])),
            }
        info: dict = {}
        for sid2 in self.sessions.keys():
            info[sid2] = self.get_session_info(sid2)
        return info

    def list_sessions(self) -> t.List[str]:
    ...
    def embed(
        self,
        input_texts: t.Union[str, t.List[str]],
        *,
    ...
    def chat_session_rag(self, sid: str, user_message: str, **opts) -> str:
        ...
        if hits:
            ...
            context = (
                "Use the following context. If it is insufficient, say you are unsure.\n---\n"
                + "\n---\n".join(parts)
                + "\n---"
            )
        ...
        if context:
            if not messages:
                ...
                # insert before last user turn
                messages[-1:-1] = [{"role": "system", "content": context}]
...
# ============================================================
# Module-level convenience API (singleton-style)
# ============================================================

_default_client: t.Optional[GPTClient] = None
...
# --- Thin wrappers for client methods ---

def chat(prompt: str, **opts) -> str:
    return get_client().chat(prompt, **opts)


def start_session(session_id: t.Optional[str] = None, system: t.Optional[str] = None) -> str:
...
def reset_session(sid: str, system: t.Optional[str] = None) -> None:
...
def get_session_messages(sid: str) -> t.List[dict]:
    return get_client().get_session_messages(sid)


def set_session_label(sid: str, label: t.Optional[str]) -> None:
    get_client().set_session_label(sid, label)


def get_session_label(sid: str) -> t.Optional[str]:
    return get_client().get_session_label(sid)


def get_session_parent(sid: str) -> t.Optional[str]:
    return get_client().get_session_parent(sid)


def get_session_children(sid: str) -> t.List[str]:
...
def embed(input_texts: t.Union[str, t.List[str]], **opts) -> dict:
    return get_client().embed(input_texts, **opts)


def set_session_vectors(sid: str, entries: t.List[dict]) -> None:
    get_client().set_session_vectors(sid, entries)


def get_session_vectors(sid: str) -> t.List[dict]:
    return get_client().get_session_vectors(sid)


def session_embed_add(sid: str, texts: t.Union[str, t.List[str]], **opts) -> t.List[int]:
    return get_client().session_embed_add(sid, texts, **opts)


def session_embed_search(sid: str, query: str, k: int = 5, **opts) -> t.List[dict]:
...
def layout_embed_add(layout_id: str, texts: t.Union[str, t.List[str]], **opts) -> t.List[int]:
    return get_client().layout_embed_add(layout_id, texts, **opts)


def layout_embed_search(layout_id: str, query: str, k: int = 5, **opts) -> t.List[dict]:
    return get_client().layout_embed_search(layout_id, query, k, **opts)


def create_checkpoint(sid: str, label: str, summary: t.Optional[str] = None, message_id: t.Optional[int] = None,
                      keep_last: int = 12, **opts) -> dict:
    return get_client().create_checkpoint(
        sid,
        label,
        summary=summary,
...
def get_checkpoints(sid: str) -> t.List[dict]:
    return get_client().get_checkpoints(sid)


def set_checkpoints(sid: str, cps: t.List[dict]) -> None:
    get_client().set_checkpoints(sid, cps)


def fork_session(sid: str, checkpoint_id: t.Optional[str] = None, **opts) -> str:
    return get_client().fork_session(sid, checkpoint_id, **opts)


def set_session_relations(parent_id: t.Optional[str], children: t.List[str], sid: str) -> None:
...
def get_session_info(sid: t.Optional[str] = None) -> dict:
    return get_client().get_session_info(sid)


def list_sessions() -> t.List[str]:
    return get_client().list_sessions()


# ============================================================
# Persistence helpers (parity with PHP persist/hydrate/remove)
# ============================================================

def persist_gpt_session(path: str, client: t.Optional[GPTClient] = None) -> None:
...
def hydrate_gpt_sessions(path: str, client: t.Optional[GPTClient] = None) -> None:
...
def remove_gpt_session(path: str) -> None:
    try:
        os.remove(path)
...
# ============================================================
# Self-test helpers (parity with PHP side)
# ============================================================

def gpt_forking_selftest(client: t.Optional[GPTClient] = None) -> dict:
    """
    Basic self-test that exercises session forking.
    """
    c = client or get_client()

    root = c.start_session(None, "You are a helpful test assistant.")
    c.add_message(root, "user", "This is the root conversation.")
    cp = c.create_checkpoint(root, "root-start")

    child = c.fork_session(root, cp.get("id"), label="child-1")
    c.add_message(child, "user", "This is the forked branch.")

    graph = c.get_session_fork_graph()
...
def gpt_full_selftest1(client: t.Optional[GPTClient] = None) -> dict:
    """
    Simple full self-test: chat + embeddings.
    """
    c = client or get_client()
    sid = c.start_session(None, "You are a test assistant.")
    reply = c.chat_session(
        sid,
        "Say hello in one short sentence.",
        model="gpt-5",
        max_tokens=64,
    )
    emb = c.embed(["alpha", "beta", "gamma"])

    return {
        "session_id": sid,
        "reply": reply,
        "emb_dims": emb.get("dims", 0),
        "emb_count": len(emb.get("vectors") or []),
    }


def gpt_full_selftest2(client: t.Optional[GPTClient] = None) -> dict:
    """
    Another self-test: RAG-style session and layout embeddings.
    """
    c = client or get_client()

    layout_id = "selftest-layout"
    c.layout_embed_add(layout_id, [
        "Python client usage docs",
        "PHP client usage docs",
        "Embedding and RAG workflow",
    ])

    sid = c.start_session(None, "You are a test assistant with RAG.")
    c.session_embed_add(sid, [
        "This is a test note about session embeddings.",
        "This is another test note about forking sessions.",
    ])

    reply = c.chat_session_rag(
        sid,
        "Explain what this test session is about in one sentence.",
        model="gpt-5",
        max_tokens=80,
    )
    ...
    return {
        ...
        "layout_id": layout_id,
        "reply": reply,
        "session_vecs": c.session_embed_count(sid),
    }


def run_test():
    print("Running GPT client self-test 1...")
    result1 = gpt_full_selftest1()
    pretty_print_json(result1)

    print("\nRunning GPT client self-test 2...")
    result2 = gpt_full_selftest2()
    pretty_print_json(result2)
