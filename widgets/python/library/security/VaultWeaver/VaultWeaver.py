...
class VaultWeaver:
	...
	def __init__(self) -> None:
		...
		# ensure ls_cache defaults exist
		gp = self.cfg.setdefault("global_policy", {})
		if "ls_cache" not in gp:
			gp["ls_cache"] = {
				"mode": "terminal",  # "terminal" | "none"
				"ttl_seconds": gp.get("session_ttl_seconds", 12 * 3600),
			}
			self._save_cfg()
	...
	def login_with_pin(self, username: str, pin: str, *, persistence: Optional[str] = None) -> SessionToken:
		...
		self._maybe_cache_ls(username, st, ls)
	...
	def _require_mk(self, username: str) -> bytes:
		"""
		Prefer unlocking MK via LS (PIN session) if policy allows, else fall back to password prompt.
		"""
		allow_via_pin = bool(self.cfg.get("global_policy", {}).get("allow_mk_via_pin", True))

		if allow_via_pin:
			st = self.resume_session(username)
			if st:
				# use cached LS (no prompt) or one-time PIN for this terminal if needed
				ls = self._unlock_ls_from_session(username)
				try:
					return self._unlock_mk_via_ls(username, ls)
				except Exception:
					# if enc_mk_ls.bin missing or decrypt fails, fall through to password
					pass

		# fallback: interactive password
	...
	# ---- LS cache helpers (per-terminal) ----

	def _cache_key_material(st: SessionToken) -> bytes:
		"""
		Key material to seal LS for this terminal session.
		Uses app pepper + {user,device,terminal,nonce} so another device
		or terminal can't reuse it even if they get the file.
		"""
		pepper = _read_pepper()
		material = f"{st.user}|{st.device_id}|{st.terminal_id}|{st.nonce}".encode()
		return hashlib.sha256(b"VW-LS-CACHE|" + pepper + b"|" + material).digest()

	def _ls_cache_file(self, username: str, st: SessionToken) -> Path:
		# same namespace as the token
		return (PATHS["SESSIONS_DIR"] / username / st.device_id / st.terminal_id / "ls.sealed")

	def _maybe_cache_ls(self, username: str, st: SessionToken, ls: bytes) -> None:
		mode = self.cfg["global_policy"].get("ls_cache", {}).get("mode", "terminal")
		if mode == "none":
			return
		k = _cache_key_material(st)
		# bind to the session's expiry so a stale file won't unlock later
		aad = f"exp:{st.expires_at}".encode()
		blob = aead_encrypt(k, ls, aad=aad)
		path = self._ls_cache_file(username, st)
		_tight_write(path, blob)

	def _try_load_cached_ls(self, username: str, st: SessionToken) -> Optional[bytes]:
		path = self._ls_cache_file(username, st)
		if not path.exists():
			return None
		# refuse if the session is already expired
		now = int(time.time())
		if now > st.expires_at or now > st.absolute_deadline:
			return None
		k = _cache_key_material(st)
		aad = f"exp:{st.expires_at}".encode()
		try:
			return aead_decrypt(k, path.read_bytes(), aad=aad)
		except Exception:
			return None

	def _clear_ls_cache(self, username: str) -> None:
		# best-effort: remove cache for the current terminal
		st = self._load_session(username)
		if not st:
			return
		f = self._ls_cache_file(username, st)
		try:
	...
	def _unlock_ls_from_session(self, username: str) -> bytes:
		...
		if not st:
			raise PermissionError("No active session")
		# 1) try cached LS (no prompt)
		cached = self._try_load_cached_ls(username, st)
		if cached:
			return cached
		# 2) fall back to a one-time PIN prompt, then cache for the remainder
		pin = getpass.getpass(prompt=f"[{username}] PIN (one-time for this terminal): ")
		ls = self._unlock_ls(username, pin)
		self._maybe_cache_ls(username, st, ls)
		return ls
	...
	# ---------- LS cache (per-terminal) ----------

	def _cache_key_material(self, st: SessionToken) -> bytes:
		"""
		Per-terminal sealing key: pepper + {user,device,terminal,nonce}.
		If someone copies the cache file to another box/terminal, this won't open.
		"""
		pepper = _read_pepper()
		material = f"{st.user}|{st.device_id}|{st.terminal_id}|{st.nonce}".encode()
		return hashlib.sha256(b"VW-LS-CACHE|" + pepper + b"|" + material).digest()
	...
	def _ls_cache_file(self, username: str, st: SessionToken) -> Path:
		# next to the session token
		return PATHS["SESSIONS_DIR"] / username / st.device_id / st.terminal_id / "ls.sealed"

	def _maybe_cache_ls(self, username: str, st: SessionToken, ls: bytes) -> None:
		# respect global policy (defaults added in __init__ below)
		mode = self.cfg["global_policy"].get("ls_cache", {}).get("mode", "terminal")
		if mode == "none":
			return
		k = self._cache_key_material(st)
		# bind to session expiry so a stale cache never re-opens
		aad = f"exp:{st.expires_at}".encode()
		blob = aead_encrypt(k, ls, aad=aad)
		path = self._ls_cache_file(username, st)
		_tight_write(path, blob)
	...
	def _clear_ls_cache(self, username: str) -> None:
		...
		if not st:
			return
		try:
			self._ls_cache_file(username, st).unlink(missing_ok=True)
		except Exception:
			pass
...
def demo():
	...
	# Make sure policy allows MK via PIN (in case config was older)
	gp = vw.cfg.setdefault("global_policy", {})
	if gp.get("allow_mk_via_pin") is not True:
		gp["allow_mk_via_pin"] = True
		vw._save_cfg()
		print("[policy] enabled allow_mk_via_pin in config")
	...
	# Optional admin policy for demo (skips if not root/admin)
	try:
	...
	print("[login] userA with PIN (one-time per terminal)")
	vw.login_with_pin(userA, pin=pinA)  # caches LS for this terminal

	print("[keychain] set & get 'gpt' (no prompts after PIN login)")
	vw.set_key("gpt", "sk-EXAMPLE-KEY-123", user=userA)    # uses cached LS
	print("  -> key('gpt') =", vw.key("gpt", user=userA))  # uses cached LS

	print("[export] keychain with passphrase 'carry'")
	pkg = vw.export_keychain(userA, passphrase="carry")    # uses cached LS

	userB = f"vw_user_{_rand_id(3)}"
	print(f"[setup] create_user {userB}")
	vw.create_user(userB, password="AnotherPass!55", pin="87654321")

	print("[login] userB with PIN (prevents any prompt on import)")
	vw.login_with_pin(userB, pin="87654321")               # caches LS for userB in this terminal

	print("[import] keychain into userB (merge)")
	vw.import_keychain(userB, pkg, passphrase="carry", merge=True, overwrite=False)  # uses cached LS

	print("  -> userB key('gpt') =", vw.key("gpt", user=userB))  # no pin needed now

	print("[files] register + encrypt a sample file (no password; MK via PIN)")
	print('Password: StrongPass!42')
	sample = PATHS["BASE"] / "sample.txt"
	sample.write_text("Top secret document.\n", encoding="utf-8")
	item_id = vw.register_file(userA, "doc1", sample, encrypt_now=True)  # MK via LS (no password)
	dec_path = vw.decrypt_file(userA, item_id)                           # MK via LS (no password)
	print(f"  -> decrypted to {dec_path}")

	print("[security] change PIN & Password (no re-encrypt of files; MK rewrap only)")
	vw.change_pin(userA, old_pin=pinA, new_pin="11223344")
	vw.change_password(userA, old_password=pwA, new_password="EvenStronger!99")

	# Prove file ops still work post password change (MK rewrap)
	print("[verify] decrypt again after password change (still no password; MK via PIN)")
	print('Password Changed to: EvenStronger!99')
	dec_path2 = vw.decrypt_file(userA, item_id)
