...
class Database:
    ...
    def trigger(self, table, field, data):
        ...
        if field == 'date_created':
            return _.resolveEpochTest(data)
        for i, r in enumerate(self.tables):
            if self.tables[i].table == table:
                for fieldX in self.tables[i].fields:
                    if fieldX.name == field:
    ...
    def create(self, table=False, fields=False):
        ...
        if os.path.isfile(self.file):
            ...
            sql = sql.replace('[n]', n)
            cursor.execute(sql)
            sql = 'CREATE TRIGGER UpdateLastTime UPDATE OF ' + nn + ' ON ' + table + '  BEGIN  UPDATE ' + table + " SET date_modified=datetime('now','localtime') WHERE id=old.id; END;"
            cursor.execute(sql)
            conn.close()
...
class DatabaseFields:
    def __init__(self, name=False, fieldType='text'):
        self.name = name
        self.info = {'name': name, 'type': fieldType, 'trigger': False, 'default': False}
