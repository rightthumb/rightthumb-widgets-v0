...
_.appInfo[focus()] = {
	...
	'description': 'Create groups by case',
	'categories': [
						'pattern',
						'patterns',
				],
...
def process( row ):
	...
	lowerCount = 0
	#**********************************************************************************************
	# add if lowerCount < 2 wait
	for i,x in enumerate(word):
		if x in _str.lowerChar:
			lastUpper = False
			thisGroup += x
			spent.append( i )
		else:
...
def action():
	...
	if not type( _.appData[__.appReg]['pipe'] ) == bool:
		...
		for i,row in enumerate(_.appData[__.appReg]['pipe']):
			record = process( row )
			if len(record):
				firsts.append( record[0] )
			data.append({ 'label': row, 'data': str( record ) })
		_.tables.register( 'data', data )
		_.tables.print( 'data', 'label,data' )
