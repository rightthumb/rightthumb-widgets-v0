	...
	for i,line in enumerate(data):
		if i+1 in funtions:
			...
			if isTab:
				...
				pre+='    '
			for line2 in insert.split('\n'):
				line2=_str.cleanBE( line2, ' ' )
				line2=_str.cleanBE( line2, '\t' )
				line2=_str.cleanBE( line2, ' ' )
				line2=_str.cleanBE( line2, '\t' )
...
def action():
	load()
	new=process(  _.switches.values('Files')[0]  )
	_.saveText(new,_.switches.values('Files')[0])
def load():
	...
	global data
	global insert
	insert = ''.join(_.isData(r=1))
	injection_ID = _md5.string(insert)
