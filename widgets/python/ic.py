...
def sw():
	...
	_.switches.register('ImportAs', '-as', '-as _v -i app.py imp.py')
	_.switches.register('StandaloneDestination', '-nofw,-noFW,-sd,-standalone,-nf,--not-framework')
...
def make_import4(path_consumer: str, path_provider: str) -> str:
	"""
...
import os
aliasesFi=_.aliasesFi
FW = _

# def make_import_exfer
...
def make_import(path_consumer,path_provider):
	global FW
	path_consumer=aliasesFi(path_consumer)
	# print(_)
	path_provider=aliasesFi(path_provider)
	consumer_dir = os.path.dirname(os.path.abspath(path_consumer))
	provider_abs = os.path.abspath(path_provider)

	if not path_consumer.endswith(".py") or not path_provider.endswith(".py"):
		return os.path.relpath(provider_abs, consumer_dir)

	provider_dir = os.path.dirname(provider_abs)
	provider_filename = os.path.basename(provider_abs)
	module_name, zz = os.path.splitext(provider_filename)

	is_init = (module_name == "__init__")

	# Compute what directory must be on sys.path and what import should be.
	if is_init:
		# provider_dir is the package folder (the folder containing __init__.py)
		pkg_folder = provider_dir                       # .../_rightThumb/_vars
		pkg_name = os.path.basename(pkg_folder)         # _vars

		parent_folder = os.path.dirname(pkg_folder)     # .../_rightThumb
		parent_name = os.path.basename(parent_folder)   # _rightThumb

		# Put the parent of _rightThumb on sys.path (so `_rightThumb` is importable)
		sys_path_target = os.path.dirname(parent_folder)  # .../python

		# Import as nested package so internal imports like `_rightThumb._construct` work
		# import_stmt = f"from {parent_name} import {pkg_name}  # noqa: F401"
		if FW.switches.isActive('ImportAs'):
			AS = FW.switches.values('ImportAs')[0]
			import_stmt = f"from {parent_name} import {pkg_name} as {AS}"
		else:
			import_stmt = f"from {parent_name} import {pkg_name}"
		# alt: import_stmt = f"import {parent_name}.{pkg_name} as {pkg_name}"
	else:
		# Regular module file foo.py:
		# Put the folder containing foo.py on sys.path, then import foo
		sys_path_target = provider_dir
		# import_stmt = f"import {module_name}  # noqa: F401"
		if FW.switches.isActive('ImportAs'):
			AS = FW.switches.values('ImportAs')[0]
			import_stmt = f"from {module_name} import {module_name} as {AS}"
		else:
			import_stmt = f"from {module_name} import {module_name}"
	try:
		import_stmt = import_stmt+'   # type: ignore'
	except: pass
	rel_target = os.path.relpath(sys_path_target, consumer_dir)
	steps = [] if rel_target == "." else rel_target.split(os.sep)

	join_expr_parts = ["os.path.dirname(__file__)"] + [repr(s) for s in steps]
	join_expr = ", ".join(join_expr_parts)
	sys_path_line = f"sys.path.append(os.path.abspath(os.path.join({join_expr})))"
...
def LoadIntCode():
	...
	ic.functions = {}
	ic.classes = {}
	from os import path.normpath as normpath
	try:
		_v
	except:
		_v = Meta_Namespace()
...
Loader_intelligent_code = Loader_FW_Native
...
def action():
	...
	for path in isData:
		...
		if True:
			...
			# print(files)
			if _.switches.isActive('StandaloneDestination'):
				adjust_For_Off_Framework = []
				adjust_For_Off_Framework.append('# ')
				adjust_For_Off_Framework.append('import sys, os')
			_.pyColor(iCode)
			# print(iCode)
