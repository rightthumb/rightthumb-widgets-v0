...
def epoch_to_readable(epoch):
    ...
    # Format datetime object to a readable string format, e.g., "YYYY-MM-DD HH:MM:SS"
    readable_datetime = dt.strftime('%Y-%m-%d %H:%M:%S')
...
def action():
	...
	_.pr(path,c='green')
	latest = 0
	oldest = 999999999999999999999999999
	record = {}
	for rec in log:
		if rec['file'] == path:
			if rec['timestamp'] > latest:
				latest = rec['timestamp']
			if rec['timestamp'] < oldest:
				record = rec
				oldest = rec['timestamp']
	if record:
		_.pr(epoch_to_readable(record['timestamp']),c='cyan')
