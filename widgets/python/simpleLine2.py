...
def process( row ):
	...
	for i,c in enumerate(cat):
		if c in row and end in row:
			category = row.split(c)
			if 'heal' in c:
				new = category[0] + _.colorThis( c, 'green', p=0 ) + category[1]
			if 'hurt' in c:
				new = category[0] + _.colorThis( c, 'red', p=0 ) + category[1]
			if 'help' in c:
				new = category[0] + _.colorThis( c, 'cyan', p=0 ) + category[1]

			subject = row.split(c)[1].split(end)[0]
			subject_Color = _.colorThis( subject, 'yellow', p=0 )
			new = new.replace( subject, subject_Color )

			subject = row.split(end)[1]
			if subject.startswith('http'):
				subject_Color = _.colorThis( subject, 'bold', p=0 )
			else:
				subject_Color = _.colorThis( subject, 'purple', p=0 )
			new = new.replace( subject, subject_Color )
			
			if '(' in row and ')' in row:

				subject = row.split('(')[1].split(')')[0]
				subject = '('+subject+')'
				subject_Color = _.colorThis( subject, 'bold', p=0 )
				new = new.replace( subject, subject_Color )
