...
def sw():
	...
	_.switches.register( 'Save', '-save','file.py' )
...
def action():
	_.pr('Only from clipboard',c='Background.yellow')
	fo = _v.pro+_v.slash+'ideas'+_v.slash
	_v.mkdir(fo)
	if _.switches.isActive('Save'):
		epoch = str(time.time()).split('.')[0]
		fi = fo+epoch+'--'+_.switches.values('Save')[0]
		_paste = _.regImp( __.appReg, '-paste' )
		idea = _paste.imp.paste()
		_.saveText(idea,fi)
		_.pr(idea,c='cyan')
