...
class ShellScriptValidator:
	...
	def extract_and_replace_regex(self):
		# Shell does not often have regex constructs in the same way, but we can handle special cases
	...
	def parse_assets(self):
		...
		self.assets['functions'] = re.findall(r'function\s+(\w+)\s*\(', self.code)
	...
	def validate(self):
		# Here we can implement validation logic for shell syntax, regex, and other constructs
		return True
...
def action():
	for path in _.isData(r=1):
		...
		validator = ShellScriptValidator(code)
