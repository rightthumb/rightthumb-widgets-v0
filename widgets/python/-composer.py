...
def sw():
	...
	_.switches.register( 'RecursivePaths', '-rp', '3 (depth)' )
	_.switches.register( 'RelativePaths', '-rel' )
...
def __path(path):
	if _.switches.isActive('RelativePaths'):
		return path
	return __.path(path)


def action():
	if _.switches.isActive('RecursivePaths'):
		_.switches.set('Recursive', 'active', True)
		_.switches.set('Recursive', 'value', _.switches.value('RecursivePaths'))
		_.switches.set('Recursive', 'values', _.switches.values('RecursivePaths'))
	if _.switches.isActive('IncludeCommands'):
		include_lines = generate_include_commands()
		for line in include_lines:
			_.pr(line, c='cyan')
	...
	# 👉 NEW: show folder + its commands when using -r
	if _.switches.isActive('Recursive'):
		output = []
		paths = []
		for path, commands in results.items():
			folder = os.path.dirname(path)
			if not _.switches.isActive('Plus') and not _.switches.isActive('RecursivePaths'):
				_.pr(f'\n{__path(folder)}', c='yellow')   # folder header
			paths.append(__path(folder))
			
			for cmd in commands:
				cnt += 1
				if _.switches.isActive('Plus') or _.switches.isActive('RecursivePaths'):
					if _.showLine(cmd):
						if _.switches.isActive('Plus'):
							# _.pr(f"require '{__path(folder)}/vendor/autoload.php';", c='yellow')
							output.append(f"require '{__path(folder)}/vendor/autoload.php';")
							
							# _.pr(f'\n{__path(folder)}', c='yellow')
							break
				if not _.switches.isActive('Plus') and not _.switches.isActive('RecursivePaths'):
					_.pr(f'  {cmd}', c='cyan')       # indented commands
		
		if _.switches.isActive('RecursivePaths'):
			a = sorted(set(paths), key=lambda p: len(p.strip('/').split('/')))
			b = sorted(paths, key=lambda p: (p.count('/'), p))
			for p in b:
				_.pr(p, c='yellow')
		elif not _.switches.isActive('RecursivePaths'):
			if output:
				paths_sorted = sorted(output, key=lambda p: len(p.strip('/').split('/')))
				_.pr(paths_sorted[0], c='yellow')
		if not _.switches.isActive('Plus') and not _.switches.isActive('RecursivePaths'):
			_.pr('\n# Total: ', cnt, c='yellow')
		return

	# 👉 original behavior (non-recursive)
	_commands = []
	seen = set()
