...
def appSwitches():
	pass
	_.switches.register( 'Delim', '-delim', '" "', isRequired=True )
	_.switches.register( 'Colors', '-color,-colors', 'white,green', isRequired=True )
...
_.appInfo[focus()] = {
	'file': 'colorizeSegment.py',
	'liveAppName': __.thisApp( __file__ ),
	'description': 'Changes the color of deliminiated segments',
	'categories': [
						'color',
						'delim',
				],
	...
	'examples': [
						'p dir -cache %mData% -movietitle -movies -ago 2m | p imdb -case -ent -score -skipload | p line -u --c | sort /R | p colorizeSegment -delim " " -colors white green *cyan ',
						'p dir -cache %mData% -movietitle -movies -ago 2m | p line --c -make "p imdb -score -qi -ent {}" | p execute --c | sort /R | p colorizeSegment -delim " " -colors white green *cyan ',
						''
	],
	'columns': [
...
def process( row ):
	record = row.split( _.switches.values('Delim')[0] )
	colorMax = len( _.switches.values('Colors') )-1
	theRestActive = False
	theRestColor = ''
	for i, segment in enumerate(record):
		if i <= colorMax:
			if '*' in _.switches.values('Colors')[i]:
				theRestActive = True
				theRestColor = _.switches.values('Colors')[i].replace( '*', '' )
			else:
				record[i] = _.colorThis( segment, _.switches.values('Colors')[i], p=0 )
		if theRestActive:
