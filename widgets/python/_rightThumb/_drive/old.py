...
class Documentation_Initial:
	...
	def __init__( self, letter ):
		...
		def validate():
			...
			if not self.type == '' and not self.label == '' and not self.priority == '' and self.isInt( self.priority ):
		...
		def suggestName():
			...
			# theDriveSize = self.formatSizeInt( int(self.driveSizeLabel) )
			theDriveSize = self.volumeSizeTotalInt


			if self.type == 'archive':

				if not self.owner == 'me':
	...
	def volumeSizeInfo( self ):
		...
		self.volumeSizeTotalInt = self.formatSizeInt(total)
		self.volumeSizeTotal = self.formatSize(total)
		self.volumeSizeUsed = self.formatSize(used)
		self.volumeSizeFree = self.formatSize(free)
		self.driveSize = self.estimateDriveSize(total)

		if not 'g' in self.volumeSizeTotal.lower():
			self.driveSizeLabel = self.formatSize(self.driveSize)
		else:
	...
	def removeNumbers( self ):
		result = ''
		n = '0123456789.'
		for ch in self.volumeSizeTotal:
			if not ch in n:
				result += ch
		return result
	...
	def formatSizeB( self, size ):
		...
		if self.isInt(result):
			result = int(result)
		elif self.isFloat(result):
			result = float(result)
		else:
			result = 0
		return result

	def formatSizeInt( self, size ):
		# size = int(size)
		try:
			size = int(size)
		except Exception as e:
