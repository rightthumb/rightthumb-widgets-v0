...
class pID:
	def __init__( self ):
		...
		self.spent=[]
		# self.preCheck()
	def pattern( self, pattern, split=None ):
		if split is None and type(pattern) == list:
			self.code = pattern
	...
	def skipSome( self, miniID ):
		if not type(miniID) == list:
			if not split is None:
				miniID = miniID.split(split)
		if not self.hasRun:
			self.gen(0)
		self.hasRun = True
		if miniID is None:
	...
	def resolve( self, miniID=None, split=None ):
		if not type(miniID) == list:
			if not split is None:
				miniID = miniID.split(split)
		if not self.hasRun:
	...
	def gen( self, num=189922, data=False, d=None, err=False ):
		if type(num) == str:
			numX = ''
			for x in num:
				if x in '0123456789':
		...
		if perfection:
			...
			data.sort()
			dataA=' '.join(data)
			if dataA in self.spent:
				perfection=-1
			else:
				self.spent.append(dataA)
	...
	def place_value_id( self, test=189922, save=False ):
		...
		for i in range(1,self.max):
			...
			try:
				...
				self.max=i-1
				return self.place_value_id( test=test, save=save )
			if not hitMax:
				if save:
					try:
