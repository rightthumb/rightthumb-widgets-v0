...
def login( label, appReg=None ):
	...
	_.saveText( _v.enTest, _blowfish.encrypt( 'abc123', password ) )
	return password


def testDecrypt( password, default=False ):
	if not os.path.isfile( _v.enTest ): return default
	en = open( _v.enTest, 	'r' ).read()
	de = string_de( en )
	if de == 'abc123':
		return True
	return False
...
def key(password=None):
	...
	if os.path.isfile(auto):
		...
		if password is None:
			...
			while password == '':
				password = getpass.getpass()
				if not testDecrypt( password, True ):
					_.colorThis( 'Invalid password', 'red' )
					change = ''
					while change == '':
						change = _.askYesNo( 'Change password' )
						if change == 'yes':
							_.pr()
							_.colorThis( 'Vault:', 'yellow' )
							password = ''
