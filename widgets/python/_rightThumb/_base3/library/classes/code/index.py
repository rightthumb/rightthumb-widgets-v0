...
class index:
	...
	def color(self):
		...
		for m in methods:
			testMethods['python'].append('.'+m+'(')
		words_to_colorize = [
			# Common prepositions and conjunctions
			'in', 'from', 'with', 'as', 'by', 'for', 'on', 'at', 'to', 'of', 'out', 'into', 
			'about', 'over', 'under', 'between', 'around', 'through', 'up', 'down', 'before', 
			'after', 'since', 'until', 'while', 'because', 'although', 'though', 'unless',

			# Python keywords
			'def', 'class', 'return', 'import', 'from', 'as', 'if', 'elif', 'else', 'while', 
			'for', 'break', 'continue', 'pass', 'try', 'except', 'finally', 'raise', 'yield', 
			'assert', 'global', 'nonlocal', 'del', 'lambda', 'True', 'False', 'None', 'and', 
			'or', 'not', 'is',

			# # Built-in functions (optional for colorizing)
			# 'print', 'len', 'range', 'input', 'open', 'int', 'float', 'str', 'list', 'dict', 
			# 'set', 'tuple', 'bool', 'type', 'sum', 'min', 'max', 'abs', 'round', 'map', 'filter', 
			# 'sorted', 'zip', 'enumerate', 'all', 'any', 'dir', 'help', 'isinstance', 'id'
		]

		purple = ['def', 'class', 'return',]

		# words_to_colorize = [
		# 	'in', 'from', 'with', 'as', 'by', 'for', 'of', 'if', 'not', 'is',
		# ]




		dex = _.code2(self.code, lan=self.language)
		# for i in dex: print(self.code[i])
		...
		for i in dex:
			...
			if c in '{[(<':
				...
				index['individual'].append(dex[i])
			elif c in '"\'`':
				index['quotes'][i] = dex[i]
			elif c in ' 0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_.':
				index['text'][i] = dex[i]+1
			else:
				index['err'][i] = dex[i]
		...
		for o in index['text']:
			...
			for m in testMethods['python']:
				if m in txt:
					...
					mo = txt.index(m)
					me = txt.index('(')
					index['methods'][o+mo-1] = o+me-1

			if txt in words_to_colorize and not txt in purple:
				index['words'][o-1] = i-1
			if txt in purple:
				index['purple'][o-1] = i-1
		...
		isQuote = []
		# Collect quote colors
		for quote, close_quote in index['quotes'].items():



			# # all_colors[close_quote] = 'darkcyan'
			# # all_colors[quote] = 'darkcyan'
			all_colors[quote-1 ] = 'cyan'
			all_colors2[close_quote] = 'cyan'
			# # all_oc[quote] = quote
			# # all_oc[close_quote] = close_quote
			all_oc[quote-1] = close_quote
			inQuotes[quote-1] = close_quote
			endQuotes.append(close_quote)
			isQuote.append(quote)
			isQuote.append(close_quote)


			# all_colors[close_quote] = 'darkcyan'
			# all_colors[quote] = 'darkcyan'
			# all_colors[quote ] = 'cyan'
			# all_oc[quote] = quote
			# all_oc[close_quote] = close_quote
			# all_oc[quote] = close_quote - 1
		...
		# Collect method colors
		for method, color in index['methods'].items():
			all_colors[method] = 'blue'
			all_colors2[color] = 'blue'
			all_oc[method] = color

		# Collect word colors
		for word, color in index['words'].items():
			if not word in all_colors:
				all_colors[word] = 'blue'
				all_colors2[color] = 'blue'
				all_oc[word] = color

		# Collect word colors
		for word, color in index['purple'].items():
			all_colors[word] = 'purple'
			all_colors2[color] = 'purple'
			all_oc[word] = color

		# # Collect open/close character pairs
		# for oc, close_char in index['oc'].items():
		# 	all_colors[oc] = 'green'
		# 	all_oc[oc] = oc
		# 	all_oc[close_char] = close_char
		...
		last_start = 0
		last_color = 'white'
		last_color = all_colors[word]
		
		# Iterate through the code to apply colors
		for i, char in enumerate(self.code):
			if i in all_colors2:
				last_color = all_colors2[i]
			shouldBuffer = True
			# if i in index['individual']:
			# if not active and c in '{}[]()<>':
			# if not active and (char in '{}[]()<>' or char in '"\'`'):
			# if not active and (char in '{}[]()<>' or i in inQuotes or i in endQuotes or char in '"\'`'):
			if not active and (char in '{}[]()<>' or i in inQuotes or i in endQuotes or i in isQuote):
				if i in endQuotes:
					pass
					parts.append({'txt': ''.join(buffer), 'color': 'cyan', 'start': last_start, 'individual': True })
				# elif all_end:
				# 	txt = ''.join(buffer)
				# 	if last_color == 'purple' and ' ' in txt:
				# 		xx = txt.split(' ')
				# 		parts.append({'txt': txt, 'color': last_color, 'start': last_start, 'individual': True })
				# 		# for p in xx:
				# 		# 	if p in purple:
				# 		# 		# print(p, purple)
				# 		# 		parts.append({'txt': p, 'color': 'purple', 'start': last_start, 'individual': True })
				# 		# 	else:
				# 		# 		parts.append({'txt': p, 'color': 'white', 'start': last_start, 'individual': True })
				# 		# # import sys
				# 		# sys.exit()
				# 	else:
				# 		parts.append({'txt': ''.join(buffer), 'color': last_color, 'start': last_start, 'individual': True })
				else:
					parts.append({'txt': ''.join(buffer), 'color': 'white', 'start': last_start, 'individual': True })
				
				if False:
					pass
				elif char in '{}[]()<>':
					parts.append({'txt': char, 'color': 'yellow', 'start': last_start})

				elif i in isQuote:
					pass
					parts.append({'txt': char, 'color': 'darkcyan', 'start': last_start})
				elif i in inQuotes:
					if char in '"\'`':
						parts.append({'txt': char, 'color': 'darkcyan', 'start': last_start})
					else:
						parts.append({'txt': char, 'color': 'white', 'start': last_start})
					# parts.append({'txt': char, 'color': 'green', 'start': last_start})
				shouldBuffer = False
				buffer = []
				last_color = 'white'
				# continue
			...
			if shouldBuffer:
				buffer.append(char)
			if not active and i in sorted_keys and not shouldBuffer:
				active = True
				parts.append({'txt': ''.join(buffer), 'color': 'white', 'start': last_start})
				buffer = []
				last_start = i
		...
		for part in parts:
			...
			if color == 'white':
				...
				# if color == 'darkcyan':
				# 	# text += '|'
				# 	text += theColor + _.pr(part['txt'],c='darkcyan',p=0) + _.AllColor.end
				# 	# text += theColor + part['txt'] + _.AllColor.end
				# else:
				# 	text += theColor + part['txt'] + _.AllColor.end
		print(text)
