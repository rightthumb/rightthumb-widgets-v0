...
def_path__inflate_path__status = True
...
def install_module(mod: str) -> bool:
	"""
	Attempt to install a Python module using the current interpreter.

	Args:
		mod (str): The module name to install (e.g., 'requests').
	...
	try:
		...
		if result.returncode == 0:
			...
			print(f"[ERROR] Failed to install {mod}:\n{result.stderr}")
			return False
...
def imp_run2(subject):
	...
	try:
		...
		exec(f'import {subject} as imp_run__module')
	except:
		imp_run__module = None
	imp_table[subject] = imp_run__module
	return imp_run__module
...
def pathRun( p, ab=True, pop=False, file=False, slash=None, folder=None, relative=False, fi=None, fo=None, fix=True, ln=None, r=None, R=None, a=None, w=False ):
	global def_path__inflate_path__status
	if def_path__inflate_path__status: p = inflate_path(p)
	# print('___')
	# print(p)
	os = imp('os.environ')
	os = imp('os.sep')
...
def inflate_path(path):
    """
    Safely expand common path variables without requiring the path to exist.

    Supports:
        - Windows: %VAR%
        - Unix/macOS/Linux: $VAR, ${VAR}, ~, ~user

    Behavior:
        - Non-strings are returned unchanged
        - Strings without expansion markers are returned unchanged
        - If expansion raises for any reason, the original string is returned
        - If expansion does nothing, the original string is returned
