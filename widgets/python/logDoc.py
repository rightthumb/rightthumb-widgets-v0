...
def appSwitches():
	...
	_.switches.register( 'File-Documentation', '-f,-file' )
	_.switches.register( 'Activity-Documentation', '-a,-activity' )
...
def lookup( path ):
	global ad # activityDoc
	global fd # fileDoc
	global colors

	file = path
	activityDoc = ad.read( 'documentation', { 'path': file } )
	fileDoc = fd.read( 'documentation', { 'path': file } )

	if activityDoc and fileDoc:
		color = colors['both']
	elif activityDoc and not fileDoc:
		color = colors['activity']
	elif not activityDoc and fileDoc:
		color = colors['file']
	else:
		color = colors['none']
...
forms['fd'] = {
    "Config": {
        ...
        "field": {"width": 40},
        "description": "Form for documenting new or existing apps."
    },
    "App Documentation Form": [
        {"label": "App Name", "type": "text", "value": ""},
        {"label": "Path", "type": "text", "value": ""},
        {"label": "Description", "type": "text_area", "value": ""},
        {"label": "Tags", "type": "checkbox", "options": ["terminal", "web", "api", "database", "utility", "frontend", "backend"], "values": []},
        {"label": "Dependencies", "type": "text_area", "value": ""},
        {"label": "Usage Notes", "type": "text_area", "value": ""},
        {"label": "Last Edited", "type": "text", "value": ""},
        {"label": "Related URL", "type": "text", "value": ""}
    ]
}
...
forms['ad'] = {
    "Config": {
        ...
        "color": {"text": "black", "bg": "white"},
        "field": {"width": 40},
        "description": "Form for logging development activity like creating or editing an app."
    },
    "App Activity Log Form": [
        {"label": "Activity Type", "type": "dropdown", "options": ["Create App", "Edit App", "Refactor", "Bug Fix", "Documentation", "Feature Add"], "value": ""},
        {"label": "App Name", "type": "text", "value": ""},
        {"label": "Path", "type": "text", "value": ""},
        {"label": "Summary", "type": "text_area", "value": ""},
        {"label": "Date", "type": "text", "value": ""},
        {"label": "Author", "type": "text", "value": ""},
        {"label": "Files Affected", "type": "text_area", "value": ""},
        {"label": "Related URL or Ticket", "type": "text", "value": ""}
    ]
}

def addDocFile( path=None ):
	global forms
	global fd # fileDoc
	results = _.Form(forms['fd'])

def addDocActivity( path ):
	pass

from library.tools.db.sqliteMgr import  sqliteMgr
def action():
	'''
		darkcyan = no documentation
		   cyan = file has documentation
		  green = file has both documentation for file and activity
		 yellow = file has activity but no documentation for file
	...
	ad = sqliteMgr.sqliteMgr(_v.tt+__.os.sep+'logActivity__Documentation.db')
	fd = sqliteMgr.sqliteMgr(_v.tt+__.os.sep+'logiFile__Documentation.db')


	_.switches.register( 'File-Documentation', '-f,-file' )
	_.switches.register( 'Activity-Documentation', '-a,-activity' )

	if _.switches.isActive('File-Documentation'):
		addDocFile(  )

	if _.switches.isActive('Activity-Documentation'):
		pass
