...
def appSwitches():
	...
	_.switches.register('URL', '-url,-urls', group=[swGrp,'Formatting and Output'] )
	...
	_.switches.register('Copy', '-cp,-copy', group=[swGrp,'Extra Features'] )
	swGrp += 1
...
def registerSwitches( argvProcessForce=False ):
	...
	# _.switches.trigger( 'Folders', _.myFolderLocations )
	# __.SwitchesModifier.Trigger['Folder'] = _.myFolder
	# __.SwitchesModifier.Trigger['Folders'] = _.myFolder
	...
	_.switches.trigger( 'Search-For-Text-Include', _.ci )
...
def printer(path,ni=0):
	...
	if shouldDelete:
		...
		if _.switches.isActive('URL'):
			if  not os.path.isfile(path): return path
			# print()
			remote = _.remoteDic(path,'')
			# print(remote); _.isExit(__file__)
			if 'url' in remote and remote['url'].strip():
				x = _.pr( remote['url'], c='cyan', p=0 )
				print(x)
				# print({'x':x}); _.isExit(__file__)
		else:
