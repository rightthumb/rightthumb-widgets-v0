...
def fileFolderInfo( data ):
	f = data.split(_v.slash)
	f.reverse()
	n = f[0]
	f.pop(0)
	f.reverse()
	return { 'file': n, 'folder': _v.slash.join( f ) }
...
def action():
	...
	log = _.tables.returnSorted( 'data', 'd.timestamp', data )

	folder = os.getcwd()
	for record in log:
		fileFolder = fileFolderInfo( record['file'] )

		if fileFolder['folder'] == folder:
			...
			if not record['file'] in spent:
				...
				_v.mkdir(record['file'], isFile=True)
				shutil.copy( record['backup'], record['file'] )


		elif _.switches.isActive( 'Recursive' ) and fileFolder['folder'].startswith( folder+_v.slash ):
			pass
...
def load():
	global data
	data = _.getTable( 'fileBackup.json' )
