...
def action():
	...
	altTags = _.getTable( 'tempHTML01.json' )
	otherTags = {
					'outer': [],
					'inner': [],
	}
		...
		for k in rec.keys():
			...
			if tag.startswith('<'):
				...
				if not '!' in tag:
					x = tag.split('>')[0] + '>'
					tag = x
					otherTags['outer'].append( tag )
					tag = tag.replace( '<', '' )
					tag = tag.replace( '>', '' )
					otherTags['inner'].append( tag )



	# table03 = _.getTable( 'tempHTML03.json' )
		...
		for rec in data:
			...
			for k in rec.keys():
				...
				for tag in rec[k]:
					item = tag['Tag']
					if not tag['Tag'] == 'Tag':
						spentTags['outer'].append( item )
						if not '!' in item:
							item = item.replace( '<', '' )
							item = item.replace( '>', '' )
							spentTags['inner'].append( item )
...
def load():
	global data
	data = _.getTable( 'htmlTags.json' )





# Void Elements ( noclose )
...
void_elements = [ 'area', 'base', 'br', 'col', 'command', 'embed', 'hr', 'img', 'input', 'keygen', 'link', 'meta', 'param', 'source', 'track', 'wbr', 'video', 'audio', 'menuitem', 'doctype' ]
payload = []
data = []
########################################################################################
if __name__ == '__main__':
