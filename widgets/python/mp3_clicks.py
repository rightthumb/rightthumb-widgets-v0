...
def detect_clicks(audio_segment):
    ...
    # Should return a list of tuples (timestamp, click_type)
    return []

def process_clicks(clicks):
    timestamps = {'open': [], 'close': [], 'important': []}
    open_click = None

    for timestamp, click_type in clicks:
        if click_type == important:
            timestamps['important'].append(timestamp)
        elif click_type == open_close:
            if open_click is None:
                open_click = timestamp
            else:
                timestamps['open'].append(open_click)
                timestamps['close'].append(timestamp)
                open_click = None
