...
def run_script(name, path):
	...
	log_result(name, result, start, end)
	if result.returncode != 0:
		notify_error(name, result.stderr)

def log_result(name, result, start, end):
	with open("/var/log/pycron.log", "a") as f:
		f.write(f"\n[{start} - {end}] {name} | Code: {result.returncode}\n")
...
def action():
	jobs = _.getTable('jobs')
	if _.switches.isActive('Job'):
		for job in jobs:
			if job['name'] == _.switches.value('Job'):
				run_script(job['name'], job['path'])
