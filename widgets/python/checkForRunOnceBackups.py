...
def action():
	...
	hasRunOnceScheduled = False

	for ii,log in enumerate(data):

		if log['status'] == status['onceActive']:
			hasRunOnceScheduled = True

	file = _v.myVars + _v.slash+'hasRunOnceScheduled'
	
	if hasRunOnceScheduled:
		_.saveText( 'YES', file )
	else:
		_.saveText( 'NO', file )
...
def load():
	global data
	data = _.getTable('fileBackupSchedule.json')
