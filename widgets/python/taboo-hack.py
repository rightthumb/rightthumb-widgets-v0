...
_.appInfo[focus()] = {
	...
	'prerequisite': [
						...
						'save har',
						'',
						'p cat -f taboo.com.har + text game-card clearfix game-words --c - webkit html > taboo.txt',
						'',
						'p taboo-hack -f taboo.txt',
	],
	'examples': [
						_.hp('p taboo-hack -f taboo.txt'),
						'',
	],
	'columns': [
...
def extractWords(txt):
	record = {
				'subject': '',
				'omit': [],
	}
	for line in txt.split('\n'):
		if '"game-word"' in line:
			record['subject'] = extractWord(line)
		if '<li>' in line:
			record['omit'].append(extractWord(line))
	...
	if _.switches.isActive('Files'):
		for path in _.switches.values('Files'):
			for line in _.getText(path):
				...
				# _.pv(rec)
				# sys.exit()
		_.saveTableDB(taboo,'taboo.json')
