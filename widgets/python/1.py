...
def sw():
    ...
    # #e)--> examples
    _.switches.register( 'Files', '-f,-fi,-file,-files','file.txt  ||  folderAlias==FileInThatFolder.txt', isData='name', description='Files', isRequired=False )
...
def remove_triple_quoted_strings(code: str) -> str:
    result = []
...
def action():
    i = 0
    for line in remove_triple_quoted_strings(_.getText(_.switches.value('Files'), raw=True)).splitlines():
        # i += 1
        if line.strip():
            if not line.startswith('#') and ( not line.startswith(' ') and not line.startswith('\t') and not line.startswith('def ') and not line.startswith('class ')  ) and _.showLine(line):
                _.pr(line)
