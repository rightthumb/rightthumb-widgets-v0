...
def action():
	...
	if text.startswith('['):
		...
		for x in data: _copy.imp.copy(x)
	else:
		_copy.imp.copy(simplejson.dumps(data, indent=4, sort_keys=False, default=str))

def err():
	_.e('Missing Data, options:',[
		'copy list',
		'pipe list',
		'specify a file',
		])
