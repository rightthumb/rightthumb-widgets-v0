...
if True:
    ...
    class CapsCtrlWatcher:
        def __init__(self):
            ...
            self.listener = None
            self.active = []  # keep your structure
            self._build_key_sweep()          # NEW
            atexit.register(self.release_all)  # NEW
            self.cops_lock_off()

        # ---------- Sweep list for cleanup ----------
        def _build_key_sweep(self):
            fn = [f"f{i}" for i in range(1, 25)]
            arrows = ["up", "down", "left", "right"]
            nav = ["home", "end", "page up", "page down", "insert", "delete"]
            edits = ["backspace", "tab", "enter", "space"]
            mods = [
                "shift", "shift left", "shift right",
                "ctrl", "ctrl left", "ctrl right",
                "alt", "alt gr", "alt left", "alt right",
                "windows", "cmd", "super"
            ]
            locks = ["caps lock", "num lock", "scroll lock"]
            sym_row = ["`", "-", "=", "[", "]", "\\", ";", "'", ",", ".", "/"]
            self._SWEEP = set(list(ascii_lowercase) + list(digits) +
                            sym_row + fn + arrows + nav + edits + mods + locks)
        ...
        def is_caps_lock_on(self):
            ...
            if system == 'Windows':
                ...
                if not shutil.which('xset'):
                    ...
                    return None
                try:
                    output = subprocess.checkoutput(['xset', 'q']).decode()
        ...
        # ---------- Your original watcher logic (unchanged) ----------
        def watch_caps(self):
            once = False
        ...
        def _name_key(self, key):
            try:
                if hasattr(key, 'char') and key.char is not None:
                    return key.char.lower()  # e.g. 'a'
                return str(key).replace('Key.', '').lower()  # e.g. 'esc', 'ctrl_l'
            except Exception:
                return str(key).replace('Key.', '').lower()

        # ---------- pynput callbacks ----------
        def on_press(self, key):
            KEY = self._name_key(key)
            if key == kb.Key.esc and KEY not in self.active:
        ...
        def on_release(self, key):
            ...
            # Clean any stale keys (copy to avoid mutating while iterating)
            for k in list(self.active):
                try:
                    if not _.Pressed.key(k):
                        self.active.remove(k)
                except Exception:
                    # If _.Pressed.key fails for a token, drop it
                    self.active.remove(k)
            ...
            if key == kb.Key.esc and self.esc_pressed_time:
                ...
                if duration > 0.5:
                    # Release EVERYTHING before exiting
                    self.release_all()
        ...
        def release_all(self):
            # Release everything you tracked
            for k in list(self.active):
                try:
                    keyboard.release(k)
                except Exception:
            ...
            # Sweep common keys (catch ones not tracked or stuck)
            for k in self._SWEEP:
                try:
        ...
        def stop(self):
            ...
            try:
                ...
                self.release_all()  # ensure a clean slate
            sys.exit(0)
        ...
        def run(self):
            ...
            try:
                with kb.Listener(on_press=self.on_press, on_release=self.on_release) as self.listener:
                    _.pr("Running... \nPress Esc (hold >0.5s) to exit.", c='yellow')
                    self.listener.join()
            finally:
                # Covers normal interpreter shutdown / unexpected exceptions
                self.release_all()
