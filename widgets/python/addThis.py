...
def sw():
    ...
    # #e)--> examples
    _.switches.register( 'GroupStartsWithSplit', '-gs,--s,-g' )
    _.switches.register( 'NumSplit', '-ns,--n,-num' )
    _.switches.register( 'Raw', '-r,--raw' )
    _.switches.register( 'Parent', '-p' )
...
def action0():
    sp = _.switches.value('GroupStartsWithSplit')
    ns = _.ci(_.switches.values('NumSplit')[0])
    ni = int(_.switches.values('NumSplit')[1])
    groups = {}
    last = ''

    p = _.ci(_.switches.values('Parent')[0])
    pi = int(_.switches.values('Parent')[1])

    parents = []
    data = _.isData(2)
    for line in data:
        if not sp in line: continue
        if not ns in line: continue
        if  not _.showLine(line): continue
        # print(line)
        # print(len(line.split(p)),p)
        try:
            par = line.split(p)[pi].split(' ')[0]
            if not par in parents: parents.append(par)
            pass
            # print(line)
    ...
    for rents in parents:
        ...
        for line in data:
            ...
            if not gp in groups: groups[gp] = []
            
            try:
                num = float(line.split(ns)[ni])
            except Exception as e:
                pass
            groups[gp].append(num)
        # total nums
        total = 0
        for gp in groups:
            _.pr()
            ...
            _.pr( round(t,2) )
        _.pr()
        _.pr('total of', len(groups.keys()), 'groups')
        _.pr( _.addComma(round(total,2)) )
...
def action():
    ...
    for line in data:
        ...
        try:
            par = line.split(p)[pi].split(' ')[0].strip()
            if par.strip() and par not in parents:
                parents.append(par)
        except Exception:
            pass

    for rents in parents:
        groups = {}     # reset per parent
        last = ''
        total = 0
        ...
        for line in data:
            ...
            try:
                raw_num = line.split(ns)[ni].strip()
                num = float(raw_num)
                groups[gp].append(num)
            except Exception:
                pass

        # print totals for this parent
        for gp in groups:
            _.pr()
            _.pr(gp)
            t = sum(groups[gp])
        ...
        _.pr('total of', len(groups.keys()), 'groups')
        _.pr(_.addComma(round(total, 2)))
        _.pr('[',_.addComma(round(total, 2)),']',rents)
