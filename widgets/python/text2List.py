...
_.appInfo[focus()] = {
	...
	'examples': [
						'p text2List -file file.txt',
						'type %tmpf0% | p text2List',
						'',
						'type %tmpf3% | p text2List + .* -u',
						'',
						'type %tmpf4% | p line + ;. ;\'. -p ;\' 3 --c | p line -p " " 0 -make " .{0}" | p text2List',
						'',
	],
	'columns': [
	...
	if not type( _.appData[__.appReg]['pipe'] ) == bool:
		...
		for i,row in enumerate( _.appData[__.appReg]['pipe'] ):
			...
			if _.switches.isActive('Unique'):
				if len( row ):
					if _.showLine( row ) and not row in spent:
						spent.append( row )
						data.append( row )
		...
		_.printVar1( data )
		_.saveTable( data, 'text2List.json' )
