...
def action():
	global apps
	app_list = _v.stmp + _v.slash+'app_list.txt'
	app_list2 = _v.stmp + _v.slash+'app_list2.txt'

	

	for row in _.getText( app_list ):
		if 'HKEY_LOCAL_MACHINE' in row:
			row = row.replace( '\n', '' )
			r = row.replace( 'HKEY_LOCAL_MACHINE\\Software\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall'+_v.slash , '' )
			if not '{' in r:
				r = clean( r )
				if not hasApp( r ):
					apps.append({ 'app': r })

	for row in _.getText( app_list2 ):
		if 'REG_' in row and not '(Default)' in row:
			row = row.replace( '\n', '' )
			row = row.replace( '\t', ' ' )
			r = row.split( 'REG_' )[0]
			r = clean( r )
