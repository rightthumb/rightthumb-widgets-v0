...
def appSwitches():
	...
	_.switches.register( 'Config', '-fig,-config', 'cpanel cp whm etc' )
...
def scan_address(address):
	...
	if scan:
		...
		elif _.switches.isActive('Config'):
			stuff = _.switches.values('Config')[0].lower()
			if stuff in __.FIG:
				ports = __.FIG[stuff]
				print(ports)
...
cpanel_whm = '20,21,22,25,26,53,80,110,143,443,465,587,993,995,2077,2078,2082,2083,2086,2087,2095,2096'
spent_ports = {}

__.FIG = {


					'cpanel': PC(cpanel_whm),
					'whm': PC(cpanel_whm),
					'w': PC(cpanel_whm),
					'cp': PC(cpanel_whm),
