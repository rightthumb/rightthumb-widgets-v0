...
function update_apt() {
    ...
    run_command "apt install python3-pip3 -y"
    run_command "apt-get update python3 -y"
    run_command "apt update $PY-pip3 -y"
    run_command "-H pip3 install --upgrade pip3 -y"
...
function initializer() {
    ...
    if ! which wget > /dev/null; then
        echo -e "wget -q not found! Install? (y/n) \c"
        read
        if "$REPLY" = "y"; then
            run_command "apt-get install wget"
        fi
    fi
    if ! which python3 > /dev/null; then
        ...
        read
        if "$REPLY" = "y"; then
            run_command "apt-get install python3"
        fi
    fi
    ...
    if ! which unzip > /dev/null; then
        echo -e "unzip not found! Install? (y/n) \c"
        read
        if "$REPLY" = "y"; then
            run_command "apt-get install unzip"
        fi
    fi
    # p='simplejson'
