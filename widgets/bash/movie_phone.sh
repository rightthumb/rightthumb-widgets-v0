...
input_file="$1"
output_file="${input_file%.*}_for_phone.mp4"
...
if [ "$width" -le "$target_width" ] && [ "$height" -le "$target_height" ]; then
	echo "Input video is already optimized for a large phone. Copying video stream..."
	ffmpeg -i "$input_file" -c:v copy -c:a aac "$output_file"
else
	echo "Converting video for optimal large phone viewing..."
