...
# Disable all plugins to prevent conflicts during the update
echo "Disabling all plugins..." >> $LOG_FILE
timeout 300 $WP_CLI plugin deactivate --all --debug >> $LOG_FILE 2>&1
log_timeout
...
# Re-enable all plugins after the update
echo "Re-enabling all plugins..." >> $LOG_FILE
timeout 300 $WP_CLI plugin activate --all --debug >> $LOG_FILE 2>&1
log_timeout
